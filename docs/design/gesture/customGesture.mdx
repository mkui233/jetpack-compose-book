...
- **panZoomLock**(可选)： 如果 panZoomLock 为真，则仅当在平移或缩放运动之前检测到旋转的触摸倾斜时才允许旋转。 如果没有，将检测到平移和缩放手势，但不会检测到旋转手势。 如果 panZoomLock 为 false，则一旦达到触摸倾斜，就会检测到所有三个手势
        ...
        Box(Modifier
            ...
            .pointerInput(Unit) {
                detectTransformGestures(
                    panZoomLock = true, // 平移或缩放之后检测到旋转的倾斜时才不允许旋转
